...
for cell_ in ca1_pyr_cells: # to | from
    for pregid in cell_._presyn_list:
        if pregid < 2*n_pyr_ca1: # from Pyramidal
            ...
            pp = mt_.pp_begin(sec=target_sec)
            nc_ = pc.gid_connect(pregid, pp)
            nc_.weight[0] = settings.w_CA3_CA1[0]*2.0
            nc_.threshold = settings.syn_threshold
            nc_.delay = settings.syn_delay
            cell_._ncs.append(nc_)
...
if rank == 0:
    ...
    with open(os.path.join(dirs["save_dir"], "output.txt"), "w") as f:
        ...
        f.write("Je refais les expériences w_sca * 2 et offset (0, 50) pour un courant oscillatoire à 0.02nA")
        ...
        f.write("\nsca - pyr weight : {}\n".format(settings.w_CA3_CA1[0]*2.0))
    ...
    save_specgram(os.path.join(dirs['figures'], 'specgram_0_50.png'), [tv_pyr, tv_bc, tv_olm, tv_sca], [fv_pyr, fv_bc, fv_olm, fv_sca], [pspec_pyr, pspec_bc, pspec_olm, pspec_sca], ["pyramidal cells", "basket cells", "olm cells", "schaffer collaterals"], ylim=[0, 150], **git_kwargs)
