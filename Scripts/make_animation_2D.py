...
# upper_line = []
# for i in range(101):
#     x, y = get_flattened_coordinates(i*0.01, 1.0, shape_length, thickness)
#     upper_line.extend(np.array([[x,y]]))
# upper_line = np.array(upper_line).reshape(-1,2)

# lower_line = upper_line.copy()
# lower_line[:,1] = 0.0
...
# initial states
pyr_facecolors = [[0., 0., 0., 0.]] * len(pyr_coords)
pyr_facecolors = np.array(pyr_facecolors)

bc_facecolors = [[0., 0., 0., 0.]] * len(bc_coords)
bc_facecolors = np.array(bc_facecolors)

olm_facecolors = [[0., 0., 0., 0.]] * len(olm_coords)
olm_facecolors = np.array(olm_facecolors)

elec_facecolors = np.array([[0., 0., 0., 0.]])
...
pyr_scat = ax.scatter(pyr_coords[:,0], pyr_coords[:,1], s=200., edgecolors=cell_colors[0], facecolors=pyr_facecolors)
bc_scat = ax.scatter(bc_coords[:,0], bc_coords[:,1], s=200, edgecolors=cell_colors[1], facecolors=bc_facecolors)
olm_scat = ax.scatter(olm_coords[:,0], olm_coords[:,1], s=200, edgecolors=cell_colors[2], facecolors=olm_facecolors)
...
ani.save(os.path.join(results_dir, 'activation_20fps.mp4'), writer=FFwriter)
plt.show()
