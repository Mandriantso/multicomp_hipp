...
NEURON {
  POINT_PROCESS Izap_noisy
  RANGE delay, dur, f0, f1, amp, f, i, sigma, tau_noise
  ELECTRODE_CURRENT i
}
...
PARAMETER {
  ...
  amp (nA)
  sigma (nA) : noise amp
  tau_noise (ms) : noise update interval
...
BREAKPOINT {
  if (on==0) {
    ...
    i = amp * (sin( 2*PI * (t-delay) * (f0 + (f1 - f0)*(t-delay)/(2*dur)) * (0.001) )+1)/2 

    : Gaussian noise
    if (t - last_noise_update >= tau_noise) {
      noise = sigma * normrand(0, 1)
      last_noise_update = t
    }
    i = i + noise
  }
